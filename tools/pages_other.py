...
TRAINIUM = dict(
    ...
    eyebrow="Trainium of Thought: AWS Trainium Frontier Competition",
    h1="Model and Kernel Co-Design under a 30-Minute Training Budget",
    meta=["Ritwika Kancharla", "September 2026", "Phase 1 ongoing, currently 2nd"],
    links=[("Competition (Devpost)", "https://trainium-frontier.devpost.com/"), ("Leaderboard", "https://d3n62br6wsx257.cloudfront.net")],
    abstract="""<p>Trainium of Thought is my entry in the AWS Trainium Frontier Competition, which is run by AWS Annapurna Labs on Devpost. Phase 1 fixes the hardware (a single AWS Trainium2 chip), the dataset and the wall-clock time. Entries are ranked by the validation bits per byte of the language model that exists when 30 minutes of training have elapsed. Under this rule the number of optimizer steps is not fixed, so each design decision affects both how much the model learns per step and how many steps fit in the budget.</p>
<p>This project reduced validation bits per byte from 1.3951 to 0.9656 in full-budget local runs. The best submitted model scored 0.9848 and held first place on the public leaderboard at its peak. Phase 1 is still open, and the entry is currently second on the public leaderboard (September 2026). Most of the improvement came from two changes that depend on each other: a wider and shallower model, and hand-fused kernels that shorten each training step. Several modifications that improved loss at a fixed step count produced a worse final model, and these negative results are reported.</p>""",
    body=f"""
    <h2 id="problem">Problem statement</h2>
    <p>The competition has two phases. In Phase 1, which closes on September 30, 2026, each team of one to four people trains a language model on a single Trainium2 chip within a strict 30-minute wall-clock budget. Submissions are ranked on a public leaderboard by validation bits per byte, which the organisers recompute from the model's logits with a trusted evaluation function. The ten highest-ranked teams advance to Phase 2, where they receive full Trn2 servers for scaled training. The prize pool is $40,000, and the winners are invited to present at NeurIPS 2026.</p>
